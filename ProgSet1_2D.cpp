...
class Graph {
    ...
    void addNode(double x_coord, double y_coord) {
        vector <double> values;
    ...
    double getWeight(double x1, double y1, double x2, double y2) {
        double weight = sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2));
        return weight;
    }
    ...
    for (auto nVal : nVals) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            for (int i = 0; i < nVal; i++) {
                ...
                g.addNode(x_coord, y_coord);
            }
            for (int x = 0; x < nVal-1; x++) {
                for (int y = x+1; y < nVal; y++) {
                    double weight = g.getWeight(g.nodes[x][0], g.nodes[x][1], g.nodes[y][0], g.nodes[y][1]);
                    if (1.4/pow(nVal, 0.43) > weight) {                  
